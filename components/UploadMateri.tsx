...
const UploadMateri = ({ session }: { session: any }) => {
  ...
  Swal.fire({
    ...
    customClass: {
      popup: 'bg-gray-900 text-white rounded-xl p-8 shadow-2xl w-full sm:w-96',
      title: 'text-3xl font-semibold mb-4',
      content: 'text-base mb-4',
      confirmButton: 'bg-blue-600 text-white py-2 px-6 rounded-full hover:bg-blue-500 transition-all duration-300 w-full sm:w-auto',
      cancelButton: 'bg-gray-600 text-white py-2 px-6 rounded-full hover:bg-gray-500 transition-all duration-300 w-full sm:w-auto',
      input: 'bg-gray-800 text-white border-gray-600 rounded-md py-2 px-4 focus:ring-2 focus:ring-blue-500 w-full sm:w-auto',
      textarea: 'bg-gray-800 text-white border-gray-600 rounded-md py-2 px-4 focus:ring-2 focus:ring-blue-500 w-full sm:w-auto',
    },
    background: '#1A202C',
    preConfirm: () => {
      const updatedFile = {
        ...
        uploadTime: file.uploadTime || new Date().toISOString() // Preserve the original uploadTime if available
      };
      return updatedFile;
    }
...
const getShareUrl = (fileName: string): string => {
    return `${window.location.origin}/api/downloadmateri?materi=${encodeURIComponent(fileName)}&category=${encodeURIComponent(category)}`;
};
      ...
      <section className="mt-20 bg-gray-900 py-16 px-8">
        <div className="max-w-4xl mx-auto p-8 rounded-lg shadow-xl bg-gray-800">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            >
              <input {...getInputProps()} className="hidden" required/>
...
<button
  ...
  className={`mt-6 w-full flex items-center justify-center bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-bold py-3 px-6 rounded-full transition-transform duration-300 transform hover:scale-105 disabled:opacity-50`}
>
  {isSubmitting ? (
    <svg
              ...
              {currentFiles.map((file, index) => (
                <div key={index} className="flex justify-between items-center bg-gray-700 p-4 rounded-lg">
                  ...
                  <div className="flex space-x-3">
                    <a href={getShareUrl(file.name)} className="text-blue-500 hover:text-blue-700" download>
                      <FaDownload />
                    </a>
                    <button
