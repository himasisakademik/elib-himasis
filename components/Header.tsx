...
const navItems: NavItem[] = [
    { href: "/", label: "Beranda", icon: <Globe className="w-4 h-4" />, description: "Halaman utama" },
    {
        href: "/library",
        label: "Kategori",
        ...
        <>
            <header
                className={clsx(
                    ...
                    scrolled ? "bg-slate-900/80 backdrop-blur-lg shadow-2xl shadow-slate-900/30" : "bg-slate-900/50 backdrop-blur-md"
                )}
            >
                    ...
                    <div className="flex justify-between items-center h-20">
                        <Link href="/" className="flex items-center space-x-4 group">
                            ...
                            <div className="flex flex-col">
                                <h1 className="text-2xl font-bold bg-gradient-to-r from-white via-blue-100 to-purple-200 bg-clip-text text-transparent tracking-tight">
                                    HiVerse
                                </h1>
                                <span className="text-xs text-slate-400 -mt-1 font-medium tracking-wide">Perpustakaan Digital</span>
                            </div>
                        </Link>
                                ...
                                item.isDropdown ? (
                                    <div className="relative" ref={dropdownRef} key={item.label}>
                                        ...
                                        >
                                            ...
                                            {!item.children?.some((child) => isActiveRoute(child.href)) && (
                                                <div className="absolute inset-0 bg-slate-800/50 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity" />
                                            )}
                                                ...
                                                <div className="p-3">
                                                    {item.children?.map((child) => (
                                                        <Link
                                                            ...
                                                            href={child.href}
                                                            className={clsx(
                                                                "flex items-center space-x-4 w-full p-4 rounded-2xl transition-colors group",
                                    ...
                                    <Link
                                        ...
                                        className={clsx(
                                            ...
                                            isActiveRoute(item.href) ? "text-white shadow-lg shadow-blue-500/25" : "text-slate-300 hover:text-white"
                                        )}
                                    >
                        ...
                        <button
                            onClick={toggleMenu}
                            className="lg:hidden p-3 text-slate-300"
                ...
                <div
                    id="mobile-menu"
                    className={clsx("lg:hidden overflow-hidden transition-all duration-500 ease-in-out", menuOpen ? "max-h-screen" : "max-h-0")}
                >
                    <div className="bg-slate-900/95 backdrop-blur-2xl px-4 py-6 space-y-2">
                        {[...navItems.filter((i) => !i.isDropdown), ...(navItems.find((i) => i.isDropdown)?.children || [])].map((item, index) => (
                            <Link
                                ...
                                href={item.href}
                                className={clsx(
                                    "flex items-center space-x-4 w-full p-4 rounded-2xl transition-colors",
                                    isActiveRoute(item.href) ? "bg-blue-600/20 text-white" : "text-slate-300 hover:bg-slate-800/50"
                                )}
                            >
                                <div
                                    className={clsx(
